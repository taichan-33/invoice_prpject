...
### 2.5 エラーハンドリング戦略 (Error Labeling)

**課題:**
エラー発生時に単純にリトライ（NACK）すると、同じメールが無限に再送され続け、API 制限を超過したりコストが膨れ上がるリスクがありました。

**解決策:**
「即時失敗（Fail Fast）」アプローチを採用しました。

1.  処理に失敗した場合、メールに `INVOICE_ERROR` ラベルを付与し、`TARGET` (未読) ラベルを削除します。
2.  これにより、そのメールはシステム上の検索対象から外れ、**無限ループが物理的に阻止**されます。
3.  エラーメールは日次レポート、または閾値アラートを通じて人間が検知し、手動でリカバリ（ラベル剥がし）を行います。

### 2.6 二重監視体制 (Dual Monitoring)

**課題:**
1 日 4 万件の処理を行う中で、全てのエラーをリアルタイム通知すると Slack がパンクし、逆に翌朝のレポートまで待つと大規模障害の発見が遅れるというジレンマがありました。

**解決策:**
**「概算の即時検知」** と **「正確な日次集計」** を組み合わせました。

| 機能             | 監視方式                             | タイミング             | 目的                                |
| ---------------- | ------------------------------------ | ---------------------- | ----------------------------------- |
| **閾値アラート** | メモリ内カウンター (Error Rate > 5%) | 即時 (1 時間 Cooldwon) | システム障害/大規模エラーの早期発見 |
| **日次レポート** | Gmail API / BigQuery 集計            | 翌朝 09:00             | 全件の正確な状況把握と監査          |
